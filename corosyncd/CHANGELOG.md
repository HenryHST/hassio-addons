...
- Docker build: removed `corosync-qnetd -v` from image build (exits 1, broke GHCR publish)
- CI: multi-arch manifest via `buildx imagetools` (fixes `1.0.3-aarch64 is a manifest list`)
- Removed `image:` field until GHCR manifest is reliably published (Supervisor builds locally)
